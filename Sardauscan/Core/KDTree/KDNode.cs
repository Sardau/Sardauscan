...
namespace Sardauscan.Core.KDTree
{
	...
	public class KDNode
	{
		...
		/// </summary> 
		protected internal Vector3d[] tPoints;

		/// <summary> 
		/// The array of data values. [index] 
		/// </summary> 
		protected internal Point3D[] tData;
		...
		protected KDNode(int iBucketCapacity)
		{
			// Variables. 
			...
			// Setup leaf elements. 
			this.tPoints = new Vector3d[iBucketCapacity + 1];
			this.tData = new Point3D[iBucketCapacity + 1];
		}
		#endregion
		...
		public void AddPoint(IList<Point3D> kValues)
		{
		...
		private void ExtendBounds(Vector3d tPoint)
		{
			// If we don't have bounds, create them using the new point then bail. 
			if (tMinBound == null)
			{
				tMinBound = new double[] {tPoint.X,tPoint.Y,tPoint.Z};
				tMaxBound = new double[] { tPoint.X, tPoint.Y, tPoint.Z };
				return;
			}
		...
		{
			Array.Resize<Vector3d>(ref tPoints, tPoints.Length * 2);
			Array.Resize<Point3D>(ref tData, tData.Length * 2);
		}
