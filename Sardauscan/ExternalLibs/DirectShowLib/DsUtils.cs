        ...
        public static IPin ByCategory(IBaseFilter vSource, Guid PinCategory, int iIndex)
        {
            ...
            try
            {
                ...
                while (ppEnum.Next(1, pPins, IntPtr.Zero) == 0)
                {
                    ...
                    try
                    {
                        pin_category = DsUtils.GetPinCategory(pPins[0]);
                    }
                    catch (Exception)
                    {
                        // Ignore GetPinCategory failed - unknown category for pin
                        pin_category = Guid.Empty;
                    }

                    // Is it the right category?
                    if (pin_category != Guid.Empty && pin_category == PinCategory)
                    {
                        // Is is the right index?
                        if (iIndex == 0)
                        {
